...
tsr.py
============================
Downloads the Time Series Regression archive and converts to torch tensors. The data is saved in /processed.

The load_from_tsfile_to_dataframe script was taken from:
    https://github.com/ChangWeiTan/TSRegression/blob/master/utils/data_loader.py

URL: http://timeseriesregression.org/
"""
import os
import torch
import pandas as pd
...
def convert_data():
    """ Convert to standard torch tensor format. """
    ...
    for dataset in tqdm(datasets):
        ...
        try:
            ...
            # Keep track of the original train test split idxs
            ...
            save_pickle(tensor_data, save_loc + '/data.pkl')
            save_pickle(tensor_labels, save_loc + '/labels.pkl')
            save_pickle(original_idxs, save_loc + '/original_idxs.pkl')
